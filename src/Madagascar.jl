...
RSFROOT = get(ENV, "RSFROOT", nothing)
...
function rsf_write(file::RSFFile, tag::String)
    sfmv = joinpath(RSFROOT, "bin", "sfmv")
...
function delete_rsf(tag::String)
    sfrm = joinpath(RSFROOT, "bin", "sfrm")
    fname = String[tag]
...
if RSFROOT ≠ nothing
    ...
    total_num = length(progs)
    percent = x -> round(Int, x / total_num * 100)
    my_var = 0
    for (F, S) = [(Symbol(p), p) for p in progs]
        global my_var
        my_var += 1
        print("\rMadagascar wrapper progress $(percent(my_var))% [$my_var/$total_num]...")
        @eval export $F
        @eval begin
            progname = $S
            manfile = joinpath(RSFROOT, "share", "man", "man1",
                progname * ".1")
            if isfile(manfile)
                old_stdout = stdout
