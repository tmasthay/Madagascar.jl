...
if "RSFROOT" in keys(ENV)
    RSFROOT = ENV["RSFROOT"]
    lib_path = joinpath(RSFROOT, "lib", "libdrsf.so")
    libdrsf = Libdl.dlopen(lib_path)
    println("Using RSFROOT=$RSFROOT with Madagascar install from source")
else
    import Madagascar_jll: artifact_dir, libdrsf
    RSFROOT = artifact_dir
end
